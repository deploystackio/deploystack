...
import { getDbStatus, getDb, authUser, authSession } from '../db';
...
// Note: This is a fallback for library initialization where no logger is available
function logInfo(message: string): void {
  if (!isTestMode()) {
    // Structured fallback logging for library initialization
    const timestamp = new Date().toISOString();
    process.stdout.write(`[${timestamp}] INFO: {"operation":"lucia_init","message":"${message}"}\n`);
  }
}
...
function initializeLucia(): Lucia {
  ...
  if (dialect !== 'postgresql') {
    throw new Error(`Unsupported database type for authentication: ${dialect}. DeployStack now requires PostgreSQL.`);
  }

  // Use existing database connection
  const db = getDb() as AnyDatabase;
  ...
  // Note: Type assertion needed because Lucia expects Date columns but we use bigint for better JS compatibility
  // This is a runtime-compatible approach as Lucia works with Unix timestamps internally
  ...
  logInfo('Lucia adapter created for PostgreSQL database');
