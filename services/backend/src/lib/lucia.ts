...
function initializeLucia(): Lucia {
  ...
  // Support all database types: sqlite, turso
  if (!dialect || !['sqlite', 'turso'].includes(dialect)) {
    throw new Error(`Unsupported database type for authentication: ${dialect}`);
  }
  ...
  // DrizzleSQLiteAdapter works with all SQLite-compatible databases (SQLite, D1, Turso)
  // since they all use the same SQL syntax and Drizzle interface
  ...
  logInfo(`Lucia adapter created for ${dialect} database with existing database instance`);
