...
import {
  ...
  type UpdateUserQueryParamsRouteRequest,
  type UserConfigUpdateSuccessResponse,
  type ErrorResponse
} from './schemas';

export default async function updateUserQueryParamsRoute(server: FastifyInstance) {
  server.patch<UpdateUserQueryParamsRouteRequest>(
    '/teams/:teamId/mcp/installations/:installationId/user-configs/:configId/query-params',
    {
      preValidation: [
      ...
      schema: {
        ...
        summary: 'Update user configuration URL query parameters',
        description: 'Updates the user-specific URL query parameters for an MCP server installation configuration. Requires Content-Type: application/json header when sending request body. Supports both cookie-based authentication (for web users) and OAuth2 Bearer token authentication (for CLI users).'
      }
    },
    async (request, reply) => {
      const { teamId, installationId, configId } = request.params;
      const userId = request.user!.id;
      const { query_params } = request.body;
      ...
      request.log.info({
        operation: 'update_mcp_user_config_query_params',
        teamId,
        installationId,
        configId,
        userId,
        authType,
        queryParamCount: Object.keys(query_params).length
      }, 'Updating MCP user configuration URL query parameters');

      try {
        const db = getDb();
        const userConfigService = new McpUserConfigurationService(db, request.log);

        const updatedConfig = await userConfigService.updateUserQueryParams(
          configId,
          userId,
          teamId,
          query_params
        ...
        try {
          ...
          }, 'Satellite commands created for user query params update');
        } catch (commandError) {
          request.log.error(commandError, `Failed to create satellite commands for user query params update ${configId}:`);
        ...
        const successResponse: UserConfigUpdateSuccessResponse = {
          ...
          message: 'User configuration URL query parameters updated successfully'
        };
        ...
        request.log.error({
          ...
          configId,
          userId
        }, 'Failed to update MCP user configuration URL query parameters');
