<!--
 * SHARED URL QUERY PARAMETERS CONFIGURATION SECTION
 *
 * This component extracts ONLY the duplicated template code for URL query parameters.
...
<template>
  <div class="space-y-4">
    ...
    <!-- Query Params Empty State -->
    <div v-else class="text-center py-12">
      <div class="inline-flex items-center justify-center w-12 h-12 rounded-full bg-gray-100 mb-4">
        <SearchCode class="h-6 w-6 text-gray-400" />
      </div>
      <h3 class="text-sm font-medium text-gray-900 mb-2">{{ $t('mcpCatalog.form.configurationSchema.urlQueryParams.emptyState.title') }}</h3>
      <p class="text-sm text-gray-500 max-w-sm mx-auto">
        {{ $t('mcpCatalog.form.configurationSchema.urlQueryParams.emptyState.description') }}
      </p>
    </div>
  </div>
</template>
