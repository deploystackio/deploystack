...
const displayTags = computed(() => {
  ...
  try {
    return JSON.parse(server.value.tags as string)
  } catch {
    return []
  }
})
...
const displayPackages = computed(() => {
  ...
  try {
    const parsed = JSON.parse(server.value.packages as string)
...
const displayRemotes = computed(() => {
  ...
  try {
    const parsed = JSON.parse(server.value.remotes as string)
...
const displayTemplateArgs = computed(() => {
  ...
  try {
    const parsed = JSON.parse(server.value.template_args as string)
...
const displayTeamArgsSchema = computed(() => {
  ...
  try {
    const parsed = JSON.parse(server.value.team_args_schema as string)
    return Array.isArray(parsed) ? parsed.filter(arg => arg != null) : []
  } catch {
    return []
  }
...
const displayUserArgsSchema = computed(() => {
  ...
  try {
    const parsed = JSON.parse(server.value.user_args_schema as string)
    return Array.isArray(parsed) ? parsed.filter(arg => arg != null) : []
  } catch {
    return []
  }
...
const displayTeamEnvSchema = computed(() => {
  ...
  try {
    const parsed = JSON.parse(server.value.team_env_schema as string)
...
const displayUserEnvSchema = computed(() => {
  ...
  try {
    const parsed = JSON.parse(server.value.user_env_schema as string)
    return Array.isArray(parsed) ? parsed.filter(env => env != null) : []
  } catch {
    return []
  }
...
const displayTeamHeadersSchema = computed(() => {
  ...
  try {
    const parsed = JSON.parse(server.value.team_headers_schema as string)
...
const displayUserHeadersSchema = computed(() => {
  ...
  try {
    const parsed = JSON.parse(server.value.user_headers_schema as string)
    return Array.isArray(parsed) ? parsed.filter(header => header != null) : []
  } catch {
    return []
  }
...
const displayTemplateQueryParams = computed(() => {
  ...
  try {
    const parsed = JSON.parse(server.value.template_url_query_params as string)
...
const displayTeamQueryParamsSchema = computed(() => {
  ...
  try {
    const parsed = JSON.parse(server.value.team_url_query_params_schema as string)
    return Array.isArray(parsed) ? parsed.filter(param => param != null) : []
  } catch {
    return []
  }
...
const displayUserQueryParamsSchema = computed(() => {
  ...
  try {
    const parsed = JSON.parse(server.value.user_url_query_params_schema as string)
    return Array.isArray(parsed) ? parsed.filter(param => param != null) : []
  } catch {
    return []
  }
...
const displayTemplateHeaders = computed(() => {
  ...
  try {
    const parsed = JSON.parse(server.value.template_headers as string)
    return Array.isArray(parsed) ? parsed.filter(header => header != null) : []
  } catch {
    return []
  }
        ...
        <div class="space-y-6">
          ...
          <DsCard title="Server Information">
            <p class="text-sm text-muted-foreground mb-6">
              {{ t('mcpCatalog.edit.serverDetails') }}
            </p>
          ...
          <DsCard title="Server Configuration">
            <p class="text-sm text-muted-foreground mb-6">
              Official MCP Registry server configuration and transport details
            </p>
          ...
          <DsCard :title="t('mcpCatalog.edit.fields.systemInfo')">
            <p class="text-sm text-muted-foreground mb-6">
              Server metadata and tracking information
            </p>
